...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    pad_h = kx.shape[0] // 2
    pad_w = kx.shape[1] // 2
    padded = np.pad(image_bw, ((pad_h, pad_h), (pad_w, pad_w)), mode='constant')
    Ix = np.zeros((H, W), dtype=np.float32)
    Iy = np.zeros((H, W), dtype=np.float32)
    for i in range(H):
        for j in range(W):
            patch = padded[i:i + 2 * pad_h + 1, j:j + 2 * pad_w + 1]
...
def get_harris_interest_points(
    ...
    if R.max() > 0:
        R = R / R.max()
    x, y, c = nms_maxpool_pytorch(R.astype(np.float32), k=k, ksize=7)
