...
class MultilabelResNet18(nn.Module):
    def __init__(self):
        ...
        self.conv_layers = nn.Sequential(*list(pretrained_resnet.children())[:-1])
        
        self.fc_layers = nn.Sequential(
            nn.Linear(pretrained_resnet.fc.in_features, 1024),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(1024, 7)
        )
        ...
        for param in self.fc_layers[0].parameters():
            param.requires_grad = False
