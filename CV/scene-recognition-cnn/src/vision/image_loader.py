...
class ImageLoader(data.Dataset):
    ...
    def get_classes(self) -> Dict[str, int]:
        ...
        if os.path.exists(self.curr_folder):
            class_folders = [d for d in os.listdir(self.curr_folder) 
...
class MultiLabelImageLoader(data.Dataset):
    ...
    def load_imagepaths_with_labels(self) -> List[Tuple[str, torch.Tensor]]:
        ...
        allowed_classes = ['coast', 'highway', 'mountain', 'opencountry', 'street']
        df_filtered = df[df.iloc[:, 0].isin(allowed_classes)]
        
        for _, row in df_filtered.iterrows():
            class_name = row.iloc[0]
            image_name = row.iloc[1]
            attributes = row.iloc[2:9].values.astype(int)
            ...
            if os.path.exists(image_path):
                ...
                img_paths.append((image_path, attribute_tensor))
