...
def generate_confusion_data(
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(loader):
            ...
            preds[start_idx:end_idx] = pred.cpu()
            targets[start_idx:end_idx] = target.cpu()
...
def generate_accuracy_data(
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(loader):
            ...
            preds[start_idx:end_idx] = pred.cpu().numpy()
            targets[start_idx:end_idx] = target.cpu().numpy()
...
def generate_accuracy_table(
    ...
    for attr_idx in range(num_attributes):
        correct = np.sum(targets[:, attr_idx] == preds[:, attr_idx])
        total = len(targets)
        accuracy_table[attr_idx] = correct / total
