...
class Argoverse(Dataset):
    ...
    def load_path_with_classes(self, split: str, data_root: str) -> Tuple[List[Tuple[str, str]], List[str]]:
        ...
        if split == 'train':
            file_range = range(0, 170)
        else:
            file_range = range(170, 200)
        ...
        for class_dir in sorted(os.listdir(data_root)):
            ...
            if os.path.isdir(class_path):
                ...
                for file_num in file_range:
                    file_path = os.path.join(class_path, f"{file_num}.txt")
                    if os.path.exists(file_path):
                        pairs.append((file_path, class_dir))
    ...
    def get_points_from_file(self, path: str) -> torch.Tensor:
        ...
        pts = torch.tensor(points_list, dtype=torch.float32)
    ...
    def pad_points(self, pts: torch.Tensor) -> torch.Tensor:
        ...
        if N >= self.pad_size:
            ...
            pts_full = torch.zeros((self.pad_size, 3))
            pts_full[:N] = pts
            first_point = pts[0:1]
            pts_full[N:] = first_point.repeat(self.pad_size - N, 1)
    ...
    def __getitem__(self, i) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        file_path, class_name = self.instances[i]
        pts = self.get_points_from_file(file_path)
