...
def get_model_and_optimizer(args) -> Tuple[nn.Module, torch.optim.Optimizer]:
    ...
    if args.arch == "SimpleSegmentationNet":
        ...
        use_ppm = args.use_ppm if hasattr(args, 'use_ppm') else True
        model = PSPNet(
            layers=args.layers,
            bins=(1, 2, 3, 6),
            num_classes=args.classes,
    ...
    if args.arch == "SimpleSegmentationNet":
        params_list.append({"params": model.layer0.parameters(), "lr": args.base_lr})
        params_list.append({"params": model.resnet.layer1.parameters(), "lr": args.base_lr})
        params_list.append({"params": model.resnet.layer2.parameters(), "lr": args.base_lr})
        params_list.append({"params": model.resnet.layer3.parameters(), "lr": args.base_lr})
        params_list.append({"params": model.resnet.layer4.parameters(), "lr": args.base_lr})
        params_list.append({"params": model.cls.parameters(), "lr": args.base_lr * 10})
    else:
        params_list.append({"params": model.layer0.parameters(), "lr": args.base_lr})
        params_list.append({"params": model.layer1.parameters(), "lr": args.base_lr})
        params_list.append({"params": model.layer2.parameters(), "lr": args.base_lr})
        params_list.append({"params": model.layer3.parameters(), "lr": args.base_lr})
        params_list.append({"params": model.layer4.parameters(), "lr": args.base_lr})
        params_list.append({"params": model.cls.parameters(), "lr": args.base_lr * 10})
        params_list.append({"params": model.aux.parameters(), "lr": args.base_lr * 10})
        if model.ppm is not None:
            params_list.append({"params": model.ppm.parameters(), "lr": args.base_lr * 10})
...
def update_learning_rate(current_lr: float, optimizer: torch.optim.Optimizer) -> torch.optim.Optimizer:
    ...
    for i in range(num_resnet_groups):
        optimizer.param_groups[i]["lr"] = current_lr
    
    for i in range(num_resnet_groups, len(optimizer.param_groups)):
