...
class PSPNet(nn.Module):
    ...
    def __replace_conv_with_dilated_conv(self):
        ...
        for layer in [self.layer3, self.layer4]:
            dilation = 2 if layer == self.layer3 else 4
            padding = 2 if layer == self.layer3 else 4
    ...
    def forward(
        ...
        #######################################################################
        N, C, H, W = x.shape
        h_out = (H - 1) // 8 * self.zoom_factor + 1
        w_out = (W - 1) // 8 * self.zoom_factor + 1
        ...
        aux_output = self.aux(aux)
        
        main_output = F.interpolate(main_output, size=(h_out, w_out), mode='bilinear', align_corners=True)
        aux_output = F.interpolate(aux_output, size=(h_out, w_out), mode='bilinear', align_corners=True)
        
        yhat = torch.argmax(main_output, dim=1)
